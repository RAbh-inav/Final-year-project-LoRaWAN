...
void
EndDeviceLorawanMac::SetNWKkey (std::string nwkkey)
{
m_nwkkey2=nwkkey;
NS_LOG_DEBUG("NETWORK KEY IN END DEVICE LORAWAN MAC"<<m_nwkkey2);
}
  ...
  if (packet != m_retxParams.packet)
    {
      ...
      msglen=packet->GetSize();
      NS_LOG_DEBUG("Message obtained in end device lorawan mac:"<<msg);
      ...
      ApplyNecessaryOptions (micTrlr,msglen,msg);
...
void
EndDeviceLorawanMac::ApplyNecessaryOptions (LorawanMICTrailer& micTrlr, uint8_t msgLen, uint8_t *msg)
{
    uint8_t B0[16];
    //uint8_t NwkKey[16];
    //uint8_t NwkKey1[16];
  ...
  //NS_LOG_DEBUG ("devaddr=m_nwkId + m_nwkAddr = " << m_address.Get());
...
//std::cout<<networkkey1<<std::endl;
//NS_LOG_DEBUG("NETWORK KEY in enddevicelorawanmac"<<networkkey);


 std::cout<<"networkkey obtained from oneshotsender in end device lorawan mac "<< m_nwkkey2<<std::endl;
    
    
    micTrlr.GenerateB0UL (B0, devaddr, FCnt, msgLen);
    

        
            
            mic_temp = micTrlr.CalcMIC (msgLen, msg, B0, m_nwkkey2);
            //NS_LOG_DEBUG("ABHINAB"<< mic_temp);
