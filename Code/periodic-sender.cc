...
void 
PeriodicSender::SetNwrkkey(std::string nwkkey)
{
m_nwkkey=nwkkey;
NS_LOG_DEBUG("NETWORK KEY in periodic sender "<<m_nwkkey);
}

void 
PeriodicSender::SetPayload(uint8_t const *buffer)
{
buff=buffer;
}
  ...
  if (m_pktSizeRV)
    {
      ...
      packet = Create<Packet> (buff,m_basePktSize + randomsize);
    }
  else
    {
      packet = Create<Packet> (buff,m_basePktSize);
    }
