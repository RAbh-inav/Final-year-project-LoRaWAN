...
{
private:
    std::string m_mic; /*  the 4 byte mic  */
    uint8_t m_key[16];
...
public:
    ...
    std::string GetMIC (void) const;
    
    void SetMIC (std::string newMIC);
    std::string CalcMIC (uint8_t msgLen, uint8_t *msg, uint8_t B0[16], std::string NwkKey);   
    
    
     /*  Bx and xNwkSIntKey need to be 16 bytes (128 bits) each, uint128_t is not guranteed to be supported */
    ...
    //void GenerateB0DL (uint8_t B0[16], uint32_t DevAddr, uint32_t FCntDwn, uint8_t msgLen);
    void GenerateB0UL (uint8_t B0[16], LoraDeviceAddress DevAddr, uint32_t FCntUp, uint8_t msgLen);
    /*void GenerateB1UL (uint8_t B1[16], uint16_t ConfFCnt, uint8_t TxDr, uint8_t TxCh, uint32_t DevAddr, uint32_t FCntUp, uint8_t msgLen);   
      LoRaWAN 1.1 Network servers use B1 and B0 */
